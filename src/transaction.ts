...
export default class Transaction {
  ...
  verifySignature(): boolean {
    ...
    if (
      this._common.gteHardfork("homestead") && new BN(this.s).cmp(N_DIV_2) === 1
    ) {
      return false;
    }
  ...
  getUpfrontCost(): util.BN {
    return new BN(this.gasLimit).imul(new BN(this.gasPrice)).iadd(
      new BN(this.value),
    );
  }
  ...
  private _implementsEIP155(): boolean {
    ...
    const vAndChainIdMeetEIP155Conditions = v === this.getChainId() * 2 + 35 ||
      v === this.getChainId() * 2 + 36;
