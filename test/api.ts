...
import { assert, assertEquals, assertThrows } from "./testing.js";
  ...
  transactions.forEach(function (tx, i) {
    if (txFixtures[i].privateKey) {
      assertEquals(
        ...
        privateToPublic(Buffer.from(txFixtures[i].privateKey, "hex")).toString(
          "hex",
        ),
      );
    }
...
Deno.test("Throws if chain/hardfork and commmon options are given", function () {
  assertThrows(
    () =>
      new Transaction(
        {},
        { common: new Common("mainnet", "petersburg"), chain: "mainnet" },
      ),
  );
  assertThrows(
    () =>
      new Transaction(
        {},
        { common: new Common("mainnet", "petersburg"), chain: "ropsten" },
      ),
  );
  assertThrows(
    () =>
...
Deno.test(
  ...
  function () {
    ...
    const fixtureTxSignedWithoutEIP155 = new Transaction(
      txData,
      { hardfork: "tangerineWhistle" },
    );
    fixtureTxSignedWithoutEIP155.sign(privateKey);

    let signedWithEIP155 = new Transaction(
      fixtureTxSignedWithEIP155.toJSON(true),
    );
    ...
    signedWithEIP155 = new Transaction(
      fixtureTxSignedWithoutEIP155.toJSON(true),
    );
    signedWithEIP155.sign(privateKey);
    assert(signedWithEIP155.verifySignature());
    assert(signedWithEIP155.v.toString("hex") !== "1c");
    ...
    let signedWithoutEIP155 = new Transaction(
      fixtureTxSignedWithEIP155.toJSON(true),
      {
        hardfork: "tangerineWhistle",
      },
    ...
    signedWithoutEIP155 = new Transaction(
      fixtureTxSignedWithoutEIP155.toJSON(true),
      {
        hardfork: "tangerineWhistle",
      },
